...
def kgd(Xs,X_tr,y_tr_in,sigma0=None,step_size=0.01, v_R2=0.05, sigma_min=1e-3, t_max=1e4, path=False, nu=np.inf):
  if path:
    sigmas=[]
    r2s=[]
    yss=[]
  ...
  for i in range(int(t_max/step_size)):
    ...
    r2=(1-np.mean((y_tr-Ih@ys)**2)/np.mean(y_tr**2))
    if path:
      sigmas.append(sigma)
      r2s.append(r2)
      yss.append(ys+y_tr_mean)

    dR2dt=get_dR2dt(y_tr, Ih@ys,kern(D_tr,sigma))
    if dR2dt<v_R2 and sigma>sigma_min:
      if r2<1-v_R2/2-0.01:
        while sigma>sigma_min:
          sigma=sigma/(1+step_size*1)
          dR2dt=get_dR2dt(y_tr, Ih@ys,kern(D_tr,sigma))
          if dR2dt>=v_R2:
            break
      else:
